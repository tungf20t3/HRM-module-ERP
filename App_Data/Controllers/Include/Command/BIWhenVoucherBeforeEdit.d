...
#IF @@view = 0 #THEN
  #IF @@action = 'Edit' #THEN
    declare @$lockedDate nvarchar(512), @$denyAccess nvarchar(512), @$checkVoucherLockedDate smalldatetime, @$checkVoucherUnit varchar(32), @$checkVoucherPurOrg varchar(32), @$checkVoucherPurGroup varchar(32), @$rlockedDate int, @$clockedDate varchar(128), @$alockedDate int, @$mlockedDate nvarchar(4000)
    select @$denyAccess = '$NotAuthorized', @$lockedDate = null, @$checkVoucherLockedDate = null

    select @$checkVoucherLockedDate = ngay_ct, @$checkVoucherUnit = ma_dvcs, @$checkVoucherPurOrg = ma_bp0, @$checkVoucherPurGroup = ma_nh0 from @@inquiry$partition$current where stt_rec = @stt_rec
    ...
    #IF @@admin <> 1 #THEN
      ...
      if @$checkVoucherPurGroup <> '' and exists(select 1 from bisyspurgrouprights where user_id = @@userID) and not exists(select 1 from bisyspurgrouprights where user_id = @@userID and ma_nh0 = @$checkVoucherPurGroup) begin
        select @message as script, @$denyAccess as message
        return
      end
    #END
